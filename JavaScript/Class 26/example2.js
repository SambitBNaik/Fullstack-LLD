...
function getMenu(time){
    console.log("I have enterded the cafe");
...
function placeOrder(item1, item2){
    ...
    const orderStatus = new Promise(function(resolve,reject){
        if(arr.includes(item1)&& arr.includes(item2)){
            ...
            reject("One of the item or all items are not availabel that you ordered");
        }
    });
    return orderStatus;
}
...
// getMenu(7).then((menu)=>{
//     console.log(menu);
//     return placeOrder("Tea","Biscuits");
// }).then((order)=>{
//        console.log(order);
//        return serve(); 
...
// async await -> promise -> consumers part been changes to handle events easily

async function cafeFlow(){
    try{
        const menu = await getMenu(7);
        const placingAnOrder = await placeOrder("Tea","Biscuits");
